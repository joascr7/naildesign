...
export default function BookingModal({
  ...
  const horariosDisponiveis = pegarHorariosDoDia(data);
  ...
  function precoFinal() {
    const temPromocao =
      ...
      Number(servicoEscolhido?.precoPromocional) < Number(servicoEscolhido?.preco);
  ...
  function dataDentroDoBloqueio(dataSelecionada, inicio, fim) {
    const dataFinal = fim || inicio;
    return dataSelecionada >= inicio && dataSelecionada <= dataFinal;
  }
  ...
  async function carregarHorarios() {
    try {
      ...
      if (snap.exists()) {
        ...
        setHorarios(Array.isArray(dados.lista) ? dados.lista : []);
      } else {
        setHorarios(["08:00", "11:00", "14:00", "17:00", "19:00"]);
      }
    } catch (e) {
      console.log(e);
      setHorarios(["08:00", "11:00", "14:00", "17:00", "19:00"]);
  ...
  async function carregarConfigAgenda() {
    const ref = doc(db, "configAgenda", "principal");
    ...
    if (snap.exists()) {
      ...
      setConfigAgenda({
        ...
        mensagemFechado: dados.mensagemFechado || "Agenda indisponível no momento."
      });
    }

    const siteRef = doc(db, "siteConfig", "principal");
    const siteSnap = await getDoc(siteRef);

    if (siteSnap.exists()) {
      const siteDados = siteSnap.data();
      setWhatsappLoja(String(siteDados.whatsapp || "").replace(/\D/g, ""));
    }
  }
  ...
  async function buscarHorariosOcupados(dataSelecionada) {
    ...
    const configDia = horariosSemana[nomeDia];

    if (configDia?.fechado) {
      setDiaBloqueado(true);
      setMensagem("Não atendemos nesse dia.");
      return;
    }
    ...
    if (configAgenda.dataInicioAgendamento && dataSelecionada < configAgenda.dataInicioAgendamento) {
      setDiaBloqueado(true);
      setMensagem(`Agendamentos disponíveis a partir de ${configAgenda.dataInicioAgendamento}.`);
      return;
    }
    ...
    const ocupados = snapAgendamentos.docs
      .map((docItem) => docItem.data())
      .filter((item) => item.status === "agendado" || item.status === "confirmado")
      .map((item) => item.horario);

    const snapBloqueios = await getDocs(collection(db, "bloqueiosAgenda"));

    const bloqueiosAtivos = snapBloqueios.docs
      .map((docItem) => ({ id: docItem.id, ...docItem.data() }))
      .filter((b) => b.ativo !== false)
      .filter((b) => dataDentroDoBloqueio(dataSelecionada, b.dataInicio, b.dataFim));

    const bloqueioDiaTodo = bloqueiosAtivos.find((b) => b.diaTodo === true);

    if (bloqueioDiaTodo) {
      setDiaBloqueado(true);
      setMensagem(bloqueioDiaTodo.motivo ? `Data indisponível: ${bloqueioDiaTodo.motivo}` : "Data indisponível.");
      return;
    }

    const bloqueados = bloqueiosAtivos.filter((b) => b.horario).map((b) => b.horario);

    setHorariosOcupados(ocupados);
    setHorariosBloqueados(bloqueados);
  }

  function pegarHorariosDoDia(dataSelecionada) {
    if (!dataSelecionada) return [];
    ...
    if (!configDia || configDia.fechado === true) return [];

    return Array.isArray(configDia.horarios) ? configDia.horarios : [];
  }
...
O que preciso fazer agora para validar a minha vaga? `
      ...
      window.location.href = `https://wa.me/55${numeroLoja}?text=${texto}`;
  ...
  useEffect(() => {
    const unsubAgenda = onSnapshot(doc(db, "configAgenda", "principal"), (snap) => {
      if (snap.exists()) {
        const dados = snap.data();
        setConfigAgenda({
          agendaAberta: dados.agendaAberta === true,
    ...
    const unsubHorarios = onSnapshot(doc(db, "configAgenda", "horariosSemana"), (snap) => {
      if (snap.exists()) {
        setHorariosSemana(snap.data());
      }
    });

    const unsubSite = onSnapshot(doc(db, "siteConfig", "principal"), (snap) => {
      if (snap.exists()) {
        const dados = snap.data();
        setWhatsappLoja(String(dados.whatsapp || "").replace(/\D/g, ""));
      }
    });
